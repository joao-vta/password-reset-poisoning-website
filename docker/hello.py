...
def index():
    if 'username' in session:
        if request.method == 'GET':
            ...
            executeQuery("INSERT INTO siteHost.posts (titulo, conteudo, link) VALUES (%s, %s, %s);", (request.form['title'], request.form['content'], myLink))
            return redirect(url_for('index'))
    else:
...
def seepost():
    ...
    postInfo = executeQuery('SELECT titulo, conteudo, link FROM siteHost.posts WHERE id=%s;', (postID,))
